	...
	public String getVideoRoom(@PathVariable("videoSeqNo") Integer videoSeqNo, Map<String, Object> map,
		...
		if (memberBean == null) {
			...
			for (CommentVideosBean commentVideosBean : commentVideoBeanList) {
				...
				List<ReplyCommentVideoBean> replyCommentVideoBeanList = replyCommentVideoService
						.getReplyCommentVideoByComment(commentVideoSeqNo);
			...
			SubscriptionUploaderBean subscriptionUploaderBean = subscriptionUploaderService
					.getSubscriptionUploader(account, uploaderAccount);
			if (subscriptionUploaderBean != null
					&& subscriptionUploaderBean.getSubscriptionUploaderStatus().equals("subscription")) {
				map.put("subscriptionUploaderStatus", subscriptionUploaderBean.getSubscriptionUploaderStatus());
			}

			// 取得喜歡影片狀況
			LikeUnlikeVideosBean likeUnlikeVideosBean = likeUnlikeVideosService.getLikeUnlikeVideos(account,
					videoSeqNo);
			...
			} else if (likeUnlikeVideosBean != null
					&& likeUnlikeVideosBean.getLikeUnlikeVideosStatus().equals("unlike")) {
				map.put("likeUnlikeVideoStatus", likeUnlikeVideosBean.getLikeUnlikeVideosStatus());
			} else if (likeUnlikeVideosBean != null
					&& likeUnlikeVideosBean.getLikeUnlikeVideosStatus().equals("none")) {
				map.put("likeUnlikeVideoStatus", likeUnlikeVideosBean.getLikeUnlikeVideosStatus());
			}
			// 取得影片評論
			List<CommentVideosBean> commentVideoBeanList = commentVideosService.getCommentVideosByVideo(videoSeqNo);
			for (CommentVideosBean commentVideosBean : commentVideoBeanList) {
				...
				for (ReplyCommentVideoBean replyCommentVideoBean : replyCommentVideoBeanList) {
					...
					ReplyCommentVideoLikeUnlikeBean replyCommentVideoLikeUnlikeBean = replyCommentVideoLikeUnlikeService
							.getReplyCommentVideoLikeUnlikeByReplyCommentAndAccount(replyCommentVideoSeqNo, account);
					if (replyCommentVideoLikeUnlikeBean != null) {
						replyCommentVideoBean.setReplyCommentVideosLikeUnlikeStatus(
								replyCommentVideoLikeUnlikeBean.getReplyCommentVideoLikeUnlikeStatus());
					}
				}
				commentVideosBean.setReplyCommentVideoBeanList(replyCommentVideoBeanList);
				CommentVideosLikeUnlikeBean commentVideoLikeUnlikeBean = commentVideoBeanLikeUnlikeService
						.getCommentVideosLikeUnlikeByCommentAndAccount(commentVideoSeqNo, account);
				if (commentVideoLikeUnlikeBean != null) {
					if (commentVideoLikeUnlikeBean.getCommentLikeUnlikeStatus() != null) {
						commentVideosBean.setCommentVideosLikeUnlikeStatus(
								commentVideoLikeUnlikeBean.getCommentLikeUnlikeStatus());
					}
				}
			}
